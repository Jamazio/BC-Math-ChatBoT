...
if "shown_resources" not in st.session_state: st.session_state.shown_resources = set()
if "custom_style" not in st.session_state: st.session_state.custom_style = ""
if "target_symbol" not in st.session_state: st.session_state.target_symbol = None
if "is_in_walkthrough" not in st.session_state: st.session_state.is_in_walkthrough = False
...
if len(st.session_state.messages) > 0:
    ...
    if not st.session_state.session_ended:
        ...
        with col2:
            if st.button("⭐ Rate BC TigerMath AI", use_container_width=True):
